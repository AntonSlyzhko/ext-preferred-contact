...
```
(node build --copy; node build --composer-install; cd site; vendor/bin/phpunit tests/unit/Espo/Modules/PreferredContact)
```
...
```
(node build --copy; node build --composer-install; cd site; vendor/bin/phpunit tests/integration/Espo/Modules/PreferredContact)
```
...
If your extension contains additional PHP packages, you also need to add `site/custom/Espo/Modules/PreferredContact/vendor` to the *scanDirectories* section in *phpstan.neon* config.
...
* `site/custom/`
* `site/client/custom/`
* `site/tests/unit/Espo/Modules/PreferredContact`
* `site/tests/integration/Espo/Modules/PreferredContact`
...
2. Set *bundled* and *jsTranspiled* to true in `src/files/custom/Espo/Modules/PreferredContact/Resources/module.json`.
3. Add `src/files/custom/Espo/Modules/PreferredContact/Resources/metadata/app/client.json`
    ```json
    {
        "scriptList": [
            "__APPEND__",
            "client/custom/modules/preferred-contact/lib/init.js"
        ]
    }
    ```
...
Add the library module path to `src/files/custom/Espo/Modules/PreferredContact/Resources/metadata/app/jsLibs.json`

```json
{
    "some-lib": {
        "path": "client/custom/modules/preferred-contact/lib/some-lib.js"
    }
}
```
